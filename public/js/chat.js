var  socket = io();
var userTypes = [];
var movingInEmojiPopup = false;
(function() {
  setEvents();
    function setEvents(){
      let messageInput = $("#message");
      messageInput.keypress(function(e){
        console.log(e.key + ' ' + e.keyCode);
        console.log($("#message").val());
        let username = $.cookie("username");
        
        if(e.keyCode === 13){
          socket.emit("stopTyping", { username: username });
        }else{
          socket.emit("typing", { username: username });
        }
      });

      messageInput.keyup(function(e){
        if($(this).val().length < 1){
          let username = $.cookie("username");
          socket.emit("stopTyping", { username: username });
        }        
      });

      let attachFileInput = $('#attach-file');
      attachFileInput.change(function(e){
        getImgData();
      });

      let emojiBtn = $('#emoji-btn');
      emojiBtn.click(function(e){
        showEmojiPopup();
      });
      // emojiBtn.mouseout(function(){
      //   closeEmojiPopup();
      // });

      let emojiDropdown = $('#emoji-dropdown');
      emojiDropdown.mouseenter(function() {
        // closeEmojiPopup();
      });
      emojiDropdown.mouseleave(function() {
        closeEmojiPopup();
      });

      let emojiArray = document.querySelectorAll(".emoji");

      $('.emoji').click(function(elem, index) {
        let username = $.cookie("username");
        let source = $(this).attr('src');
        addEmoji(username, source);
        socket.emit("chat message", {userName: username, msg: $("#message").val(), type: 'emoji', source: source});
      });
    }
    function addMessage(username, message){
      let bubbleHTML = `<div class="bubble is-in can-have-tail is-group-last" 
      data-mid="7508066303" 
      data-peer-id="-1752879275" 
      data-timestamp="1644556756">
      <div class="bubble-content-wrapper">
        <div class="bubble-content">
          <div class="message" dir="auto">${message}
            <span class="time tgico">
              <span class="i18n">12:19 PM</span>
              <div class="inner tgico" title="11 February 2022, 12:19:16">
                <span class="i18n">12:19 PM</span>
              </div>
            </span>
          </div>
          <div data-peer-id="1840063102" class="name" style="color: rgb(15, 178, 151);">
            <span class="peer-title" dir="auto" data-peer-id="1840063102">${username}
            </span>
          </div>
          <svg viewBox="0 0 11 20" width="11" height="20" class="bubble-tail"><use href="#message-tail-filled">
          <svg id="message-tail-filled" viewBox="0 0 11 20"><g transform="translate(9 -14)" fill="inherit" fill-rule="evenodd"><path d="M-6 16h6v17c-.193-2.84-.876-5.767-2.05-8.782-.904-2.325-2.446-4.485-4.625-6.48A1 1 0 01-6 16z" transform="matrix(1 0 0 -1 0 49)" id="corner-fill" fill="inherit"></path></g></svg></use></svg>
        </div>
        <avatar-element class="user-avatar avatar-40 avatar-relative" peer="1840063102" data-color="">
          <img class="avatar-photo" src="data:image/png;base64,">
        </avatar-element>
      </div>
      </div>`;

      var messages = document.getElementById("messages");
      messages.insertAdjacentHTML('beforeend', bubbleHTML);
      let messageView = document.querySelector("#messages");
      messageView.scrollTo(0, messageView.scrollHeight);
    }
    function addImage(username, source){
      let bubbleHTML = `<div class="bubble is-in can-have-tail is-group-last" 
      data-mid="7508066303" 
      data-peer-id="-1752879275" 
      data-timestamp="1644556756">
      <div class="bubble-content-wrapper">
        <div class="bubble-content">
          <div class="message" dir="auto">
            <span class="time tgico">
              <span class="i18n">12:19 PM</span>
              <div class="inner tgico" title="11 February 2022, 12:19:16">
                <span class="i18n">12:19 PM</span>
              </div>
            </span>
          </div>
          <div class="attachment media-container" style="width: 420px; height: 234px;">
            <img class="media-photo" src="${source}">
          </div>
          <div data-peer-id="1840063102" class="name" style="color: rgb(15, 178, 151);">
            <span class="peer-title" dir="auto" data-peer-id="1840063102">${username}
            </span>
          </div>
          <svg viewBox="0 0 11 20" width="11" height="20" class="bubble-tail"><use href="#message-tail-filled">
          <svg id="message-tail-filled" viewBox="0 0 11 20"><g transform="translate(9 -14)" fill="inherit" fill-rule="evenodd"><path d="M-6 16h6v17c-.193-2.84-.876-5.767-2.05-8.782-.904-2.325-2.446-4.485-4.625-6.48A1 1 0 01-6 16z" transform="matrix(1 0 0 -1 0 49)" id="corner-fill" fill="inherit"></path></g></svg></use></svg>
        </div>
        <avatar-element class="user-avatar avatar-40 avatar-relative" peer="1840063102" data-color="">
          <img class="avatar-photo" src="data:image/png;base64,">
        </avatar-element>
      </div>
      </div>`;

      var messages = document.getElementById("messages");
      messages.insertAdjacentHTML('beforeend', bubbleHTML);
      let messageView = document.querySelector("#messages");
      messageView.scrollTo(0, messageView.scrollHeight);
    }
    function addEmoji(username, source){
      let bubbleHTML = `<div class="bubble is-in can-have-tail is-group-last" 
      data-mid="7508066303" 
      data-peer-id="-1752879275" 
      data-timestamp="1644556756">
      <div class="bubble-content-wrapper">
        <div class="bubble-content">
          <div class="message" dir="auto">
            <span class="time tgico">
              <span class="i18n">12:19 PM</span>
              <div class="inner tgico" title="11 February 2022, 12:19:16">
                <span class="i18n">12:19 PM</span>
              </div>
            </span>
          </div>
          <div class="attachment"><img src="${source}" alt="🤣" class="emoji"></div>
          <div data-peer-id="1840063102" class="name" style="color: rgb(15, 178, 151);">
            <span class="peer-title" dir="auto" data-peer-id="1840063102">${username}
            </span>
          </div>
          <svg viewBox="0 0 11 20" width="11" height="20" class="bubble-tail"><use href="#message-tail-filled">
          <svg id="message-tail-filled" viewBox="0 0 11 20"><g transform="translate(9 -14)" fill="inherit" fill-rule="evenodd"><path d="M-6 16h6v17c-.193-2.84-.876-5.767-2.05-8.782-.904-2.325-2.446-4.485-4.625-6.48A1 1 0 01-6 16z" transform="matrix(1 0 0 -1 0 49)" id="corner-fill" fill="inherit"></path></g></svg></use></svg>
        </div>
        <avatar-element class="user-avatar avatar-40 avatar-relative" peer="1840063102" data-color="">
          <img class="avatar-photo" src="data:image/png;base64,">
        </avatar-element>
      </div>
      </div>`;

      var messages = document.getElementById("messages");
      messages.insertAdjacentHTML('beforeend', bubbleHTML);
      let messageView = document.querySelector("#messages");
      messageView.scrollTo(0, messageView.scrollHeight);
    }
    function getImgData() {
      let username = $.cookie("username");      
      const chooseFile = document.getElementById("attach-file");
      const files = chooseFile.files[0];
      if (files) {
        const fileReader = new FileReader();
        fileReader.readAsDataURL(files);
        fileReader.addEventListener("load", function () {
          addImage(username, this.result);
          socket.emit("chat message", {userName: username, msg: $("#message").val(), type: 'image', source: this.result});
        });    
      }
    }
    function showEmojiPopup(){
      $('#emoji-dropdown').addClass("active");
      // $('#emoji-dropdown').css('transition', 'transform .2s cubic-bezier(.4, 0, .2, 1),opacity .2s cubic-bezier(.4, 0, .2, 1);');
      $('#emoji-dropdown').css("display","");
      let offset = $('#emoji-btn').offset();
      $('#emoji-dropdown').css('left', offset.left);
    }
    function closeEmojiPopup(){
      $('#emoji-dropdown').removeClass("active");
      $('#emoji-dropdown').css('display', 'none');
    }
    function changeUserTyping(){
      if(userTypes.length === 0){
        $('#typing').text('');
        return;
      }

      if(userTypes.length === 1){
        $('#typing').text(userTypes[0] + ' is typing...');
      }else{
        $('#typing').text(userTypes.length + ' people are typing...');
      }
    }
    $("form").submit(function(e) {
      e.preventDefault(); // prevents page reloading
      let username = $.cookie("username");
      let messageStr = $("#message").val();
      if(!messageStr || !username) return;

      socket.emit("chat message", {userName: username, msg: $("#message").val(), type: 'text'});
      socket.emit("stopTyping", { username: username });
      addMessage(username, messageStr);
    
      $("#message").val("");
      console.log($.cookie('geeksforgeeks'));
    
      return false;
    });
    socket.on("received", data  =>  {
      switch(data.type){
        case 'text': addMessage(data.userName, data.msg); break;
        case 'image': addImage(data.userName, data.source); break;
        case 'emoji': addEmoji(data.userName, data.source); break;
      }     
    });
    socket.on("notifyTyping", data => {
      if(userTypes.indexOf(data.username) < 0) userTypes.push(data.username);
      
      changeUserTyping();
    });
    socket.on("notifyStopTyping", (data) =>  {
      let index = userTypes.indexOf(data.username);
      if(index > -1) userTypes.splice(index, 1);
      changeUserTyping();
    });
})()